	...
	if bot:GetMoveType() ~= MOVETYPE_LADDER then
		if bot:IsOnGround() then
			...
			if shouldClimb then
				...
				if bot:GetActiveWeapon() and bot:GetActiveWeapon().CanClimb and bot:GetActiveWeapon():IsClimbing() then
...
function D3bot.Basics.PounceAuto(bot)
	...
	if not weapon and not weapon.PounceAttack then return end
	...
	for _, pounceTargetPos in ipairs(table.Reverse(pounceTargetPositions)) do
		...
		if trajectories and (pounceTargetPos.ForcePounce or (pounceTargetPos.HeightDiff and pounceTargetPos.Pos.z - bot:GetPos().z > pounceTargetPos.HeightDiff) or timeToTarget > (trajectories[1].t1 + (weapon.PounceAttack.WindupTime or 0))*pounceTargetPos.TimeFactor) then
	...
	if (trajectory and weapon:IsIdle()) or mem.pouncing then
		if trajectory then
		...
		if not mem.pouncing then
			...
			mem.pouncingStartTime = CurTime() + (weapon.PounceAttack.WindupTime or 0)
			mem.pouncing = true
